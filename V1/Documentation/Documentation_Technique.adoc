...
[.text-justify]
De plus, la classe Client.java se trouvant dans la le package /Application/Model est très importante également.
...
. Application/View -> ComptesEditorPaneController.java, ComptesManagementController, clientseditorpane.fxml, clientsmanagement.fxml

. Application/Model -> CompteCourant.java


==== Éléments essentiels : 

[.text-justify]
L'élément essentiel en ce qui concerne la consultation d'un compte, est le controller CompteEditorPaneController.java, qui se trouve dans /Application/Control car
C'est lui qui permet de gérer la fenêtre où le guichetier consulte les comptes.

[.text-justify]
De plus, la classe CompteCourant.java se trouvant dans la le package /Application/Model est très importante également.

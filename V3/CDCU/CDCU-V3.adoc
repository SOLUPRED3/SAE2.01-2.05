...
plusieurs contraintes devront être respectées dont deux principaux types. Une bonne organisation de notre équipe sera également requise pour mener à bien le projet.


=== 1. Contraintes techniques :
[.text-justify]
Le développement de l'application présente certaines contraintes techniques devant être respectées. En effet, il faudra les suivre
à la fois pour l'utilisation finale de celle-ci, mais aussi pour les maintenances futures,
...
[.text-justify]
Pour que l'application soit fonctionnelle et pour éviter au maximum les risques d'erreurs ou de mal-fonctionnement, certaines fonctionnalités devront respecter des contraintes particulières. +
 +
*Contraintes :*
...
En effet, il est pensable que cette dernière, comme beaucoup, voire toutes les applications liées à la gestion de données personnelles,
doive respecter certaines lois.
En effet, en ce qui concerne la confidentialité et la gestion des informations relatives aux clients et à leurs comptes, elle ne peut pas faire n'importe quoi.
Nous pouvons — car c'est la principale loi qui régit aujourd'hui en Europe —
citer le RGPD (Règlement Général sur la Protection des Données). C'est un enjeu fondamental pour tout le secteur bancaire qui est fortement concerné par
l'exécution de celui-ci.
En effet, c'est en premier lieu essentiel pour les acteurs comme DailyBank, en ce qui concerne la pérennité de leur activité.
